...
Purpose
-------
1. Find the latest configured number of seasons in the NHL schedule view.
2. Read regular-season games for those seasons.
...
TEAM_TABLE = os.getenv(
    "TEAM_TABLE",
    "pacey32-agency.Team.TeamList_History",
)
...
DESTINATION_TABLE = os.getenv(
    "DESTINATION_TABLE",
    "pacey32-agency.Team.Travel_1_ScheduleExpanded_5yr",
)
...
def get_latest_seasons() -> list[int]:
    """Return the latest configured number of regular seasons."""
    query = f"""
    SELECT DISTINCT
        season
    FROM `{SCHEDULE_TABLE}`
    ...
    if len(seasons) != SEASONS_TO_INCLUDE:
        raise RuntimeError(
            f"Requested {SEASONS_TO_INCLUDE} seasons, "
            f"but found {len(seasons)}: {seasons}"
        )
...
def get_schedule(
    ...
    """Read regular-season games for the selected seasons."""
    if not seasons:
        raise ValueError(
    ...
    if schedule_df.empty:
        raise RuntimeError(
            "No regular-season games were found for "
            f"seasons {seasons}."
        )
    ...
    for column in integer_columns:
        schedule_df[column] = pd.to_numeric(
            schedule_df[column],
            errors="raise",
        ).astype("int64")
    ...
    if not duplicate_games.empty:
        raise ValueError(
            "Duplicate schedule games found:\n"
            + duplicate_games[
                [
                    "season",
                    "game_id",
    ...
    print("\nSchedule states by season:")
    print(
        schedule_df
        .groupby(
            ["season", "game_schedule_state"],
            dropna=False,
        )
        .size()
        .reset_index(name="games")
...
def get_team_list() -> pd.DataFrame:
    """Read the historical season/team location reference."""
    query = f"""
    SELECT
        season,
        team_id,
    ...
    team_df["season"] = pd.to_numeric(
        team_df["season"],
        errors="raise",
    ).astype("int64")

    team_df["team_id"] = pd.to_numeric(
        team_df["team_id"],
        errors="raise",
    ).astype("int64")
...
def build_team_locations(
    ...
    """Join each season/team row to its city coordinates."""
    duplicate_team_keys = team_df[
        team_df.duplicated(
    ...
    if not duplicate_team_keys.empty:
        raise ValueError(
            "Duplicate season/team rows found:\n"
            + duplicate_team_keys.to_string(index=False)
        )
...
def validate_team_locations(
    ...
    if not missing_keys.empty:
        raise ValueError(
            "Schedule teams missing from TeamList_History:\n"
    ...
    if not failures.empty:
        raise ValueError(
            "Season/team rows with incomplete location data:\n"
...
def main() -> None:
    ...
    print(
        "Starting Travel_1_ScheduleExpanded_5yr pipeline."
    )
    ...
    latest_seasons = get_latest_seasons()
    ...
    print(
        "\nTravel_1_ScheduleExpanded_5yr pipeline "
