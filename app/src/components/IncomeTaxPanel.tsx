    ...
    const chartData = distribution
        ...
        .map((row) => ({
            ...
            venueLocation:
                row.venueLocation,
            rate:
                row.combined_top_marginal_income_tax_rate,
            rank:
                row.income_tax_rank,
            selected:
                row.venueLocation ===
        ...
        <div className="w-full min-w-0 max-w-full space-y-8">
            ...
            <div className="grid w-full min-w-0 grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
                ...
                <TaxKpi
                    ...
                    value={`${tax.state_income_tax_rate.toFixed(
                        2
                    )}%`}
                    detail={
                        country === "Canada"
                ...
                <ChartPanel title="Rate Breakdown">
                    <div className="space-y-6 pt-3">
                        <BreakdownRow
                            label="Federal Top Rate"
                            value={
                                tax.federal_income_tax_top_rate
                            }
                            colour="bg-blue-500"
                        />
            ...
            <div className="grid w-full min-w-0 grid-cols-1 gap-4 lg:grid-cols-2">
                ...
                <div className="min-w-0 rounded-xl border border-slate-800 bg-slate-950/40 p-4 sm:p-5">
                    ...
                    <p className="mt-4 border-t border-slate-800 pt-4 text-xs leading-5 text-slate-500">
                        Rates represent top marginal
                        statutory personal income tax
                        rates. They do not represent
                        the effective tax rate paid by
            ...
            <div className="border-t border-slate-800 pt-5 text-xs leading-5 text-slate-500">
                NHL average and rankings include{" "}
                {tax.nhl_city_count} NHL cities with
                available income tax data. Rank #1
                represents the lowest combined top
...
function taxVenueLocation(
    ...
    const match = distribution.find(
        (row) =>
            row.geocoded_city === city
    );

    return (
        match?.venueLocation ?? city
    );
}
...
function formatPoints(
    ...
    if (value < 0) {
        return `${amount} pts below`;
    }

    if (value > 0) {
        return `${amount} pts above`;
    }

    return "At NHL average";
}

function formatPointsShort(
    value: number
) {
    if (
